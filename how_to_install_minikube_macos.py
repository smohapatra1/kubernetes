...
subprocess_cmd('sysctl -a | grep -E --color "machdep.cpu.features|VMX" ; brew update ; brew install minikube; sudo mv minikube /usr/local/bin ; which minikube ; minikube version; kubeadmin version ; kubectl version ; kubelet version ; minikube status ; chown -R $USER $HOME/.minikube ; sudo minikube start --vm-driver=virtualbox; minikube status ')
